    ...
    with tf.variable_scope('pose_net',reuse=flag) as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
            ...
            with tf.variable_scope('pose'):
                ...
                # cnv7a = slim.conv2d(cnv6, 64, [3, 3], stride=2, scope='cnv7a')
                # cnv7b = slim.conv2d(cnv6, 64, [3, 3], stride=2, scope='cnv7b')
                # cnv7c = slim.conv2d(cnv6, 64, [3, 3], stride=2, scope='cnv7c')
                # cnv7d = slim.conv2d(cnv6, 64, [3, 3], stride=2, scope='cnv7d')
                # pose_pred_a = slim.conv2d(cnv7a, 3 , [1, 1], scope='pred_f6_f3',
                #                         stride=1, normalizer_fn=None, activation_fn=None)
                # pose_pred_b = slim.conv2d(cnv7b, 3, [1, 1], scope='pred_f6_b3',
                #                           stride=1, normalizer_fn=None, activation_fn=None)
                # pose_pred_c = slim.conv2d(cnv7c, 3, [1, 1], scope='pred_b6_f3',
                #                           stride=1, normalizer_fn=None, activation_fn=None)
                # pose_pred_d = slim.conv2d(cnv7d, 3, [1, 1], scope='pred_b6_b3',
                #                           stride=1, normalizer_fn=None, activation_fn=None)
                # pose_avg_a = tf.reduce_mean(pose_pred_a, [1, 2])
                # pose_avg_b = tf.reduce_mean(pose_pred_b, [1, 2])
                # pose_avg_c = tf.reduce_mean(pose_pred_c, [1, 2])
                # pose_avg_d = tf.reduce_mean(pose_pred_d, [1, 2])
                # # Empirically we found that scaling by a small constant
                # # facilitates training.
                # pose_final_a = 0.01 * tf.reshape(pose_avg_a, [-1, 1, 3])
                # pose_final_b = 0.01 * tf.reshape(pose_avg_b, [-1, 1, 3])
                # pose_final_c = 0.01 * tf.reshape(pose_avg_c, [-1, 1, 3])
                # pose_final_d = 0.01 * tf.reshape(pose_avg_d, [-1, 1, 3])
                # pose_final_f6 = tf.concat([pose_final_a, pose_final_b], 2)
                # pose_final_b6 = tf.concat([pose_final_c, pose_final_d], 2)
                # pose_final = tf.concat([pose_final_f6, pose_final_b6], 1)
...
def disp_net(tgt_image, is_training=True, flag=None):
    ...
    with tf.variable_scope('disp_net', reuse=flag) as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
            ...
            upcnv7 = tf.image.resize_nearest_neighbor(cnv7b, [int(2*cnv7b.get_shape()[1].value),
                                                              int(2*cnv7b.get_shape()[2].value)])
            ...
            upcnv6 = tf.image.resize_nearest_neighbor(icnv7, [int(2 * icnv7.get_shape()[1].value),
                                                              int(2 * icnv7.get_shape()[2].value)])
            ...
            upcnv5 = tf.image.resize_nearest_neighbor(icnv6, [int(2 * icnv6.get_shape()[1].value),
                                                              int(2 * icnv6.get_shape()[2].value)])
            ...
            upcnv4 = tf.image.resize_nearest_neighbor(icnv5,[int(2 * icnv5.get_shape()[1].value),
                                                             int(2 * icnv5.get_shape()[2].value)])
            ...
            upcnv3 = tf.image.resize_nearest_neighbor(icnv4,[int(2 * icnv4.get_shape()[1].value),
                                                             int(2 * icnv4.get_shape()[2].value)])
            ...
            upcnv2 = tf.image.resize_nearest_neighbor(icnv3,[int(2 * icnv3.get_shape()[1].value),
                                                             int(2 * icnv3.get_shape()[2].value)])
            ...
            upcnv1 = tf.image.resize_nearest_neighbor(icnv2,[int(2 * icnv2.get_shape()[1].value),
                                                             int(2 * icnv2.get_shape()[2].value)])
